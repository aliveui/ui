...
import {
  ...
  CodeBlockContent,
  CodeBlockBody,
} from '@/components/ui/alive-ui/code-block';
...
const Example = () => (
  <div className="p-4">
    <CodeBlock defaultValue={code[0].language}>
      ...
      {code.map((item) => (
        <CodeBlockBody key={item.language} value={item.language}>
          <CodeBlockContent language={item.language} themes={{
            light: 'github-light',
...
import { CodeBlockContent } from '@/components/alive-ui/code-block/server';
```
